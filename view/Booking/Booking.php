...
$(document).ready(function(){
  $('#ShipmentType').change(function(){
    $('#ShipmentTypeSave').val($('#ShipmentType').val());
...
<?php
  ...
  if (isset($_POST['shipment_type_save'])){$shipment_type=$_POST['shipment_type_save'];}else{$shipment_type="CY";}
  if(isset($_POST['emp_booking'])){
    ...
    $data_array['shipment_type']=$shipment_type;
    $form=getBookingOrderFormHtml("customer",$state,$data_array);//CFS
    if(!sqlInsertIpLogActionShipmentType($_SESSION['ip'],$_SESSION['member_id'],"BookingOrder",$shipment_type)){
      "新增失敗";
    }
  }elseif($state=="update"){
    $title="修改".$text."訂艙單";
...
?>
  <form method="post" action="" id="ShipmentTypeForm" enctype="multipart/form-data">
    <input type='text' id="ShipmentTypeSave" name="shipment_type_save" value="<?php echo $data_array['shipment_type'];?>" hidden>
  </form>
  <form method="post" action="" enctype="multipart/form-data" id="BookingOrderForm">
    ...
    <!-- <input type="text" id="ShipmentType"  name="shipment_type" value="<?php echo $shipment_type;?>" hidden> -->
   ...
   <div class="row justify-content-md-center">
      ...
      <div class="col col-md-2" style="text-align:center">
        <?php
          $select_readonly="";
          if($state=="update"){
            $select_readonly=" disabled='disabled'";
          }
        ?>
        <select id="ShipmentType" name="shipment_type" class='form-select' <?php echo $select_readonly;?>>
          <?php echo getShipmentTypeOptionShipmentTypeChineseValueShipmentType($data_array['shipment_type']);?>
        </select>
      </div>
    </div>
...
if(isset($_POST['emp_booking'])){
  ...
  if ($state=="add"){
    list($data_array['serial_head'],$data_array['serial_number'])=getSerialNumber($data_array['shipment_type']);
